...
import * as productsStorage from "../storage/productsStorage";
import { z } from "zod";
...
export class ProductsController {
  async getProducts(req: Request, res: Response) {
    try {
      ...
      if (!parsed.success) {
        return res.status(400).json({ error: "Invalid params", details: parsed.error.errors });
      }
      const { search, category, limit, offset } = parsed.data;
      const { products, total } = await productsStorage.getProducts({ search, category, limit, offset });
      res.json({
        success: true,
        products,
        pagination: {
  ...
  async updateProduct(req: Request, res: Response) {
    try {
      ...
      if (!dataParsed.success) {
        return res.status(400).json({ error: "Invalid update data", details: dataParsed.error.errors });
      }
      const updated = await productsStorage.updateProduct(idParsed.data.id, dataParsed.data);
