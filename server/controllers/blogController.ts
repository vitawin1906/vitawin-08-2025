...
class BlogController {
  ...
  async createBlogPost(req: Request, res: Response) {
    try {
      ...
      const blogPostData = {
        ...
        slug: slug && slug.trim() ? `${slug.trim().toLowerCase()}-${Date.now()}` : `blog-post-${Date.now()}`,
      };
  ...
  async deleteBlogPost(req: Request, res: Response) {
    console.log(`[VDS DEBUG] deleteBlogPost called for ID: ${req.params.id}`);
    try {
      // Get admin ID - works with both auth middlewares
      ...
      console.log(`[VDS DEBUG] Validated ID: ${id}`);
