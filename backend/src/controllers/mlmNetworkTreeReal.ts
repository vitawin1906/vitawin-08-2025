...
export class MLMNetworkTreeRealController {
  /**
   * Получить дерево MLM структуры текущего пользователя (для личного кабинета)
   */
  async getUserNetworkTree(req: Request, res: Response) {
    try {
      ...
      const maxDepth = parseInt(req.query.maxDepth as string) || 16;
      const offset = parseInt(req.query.offset as string) || 0;
      const limit = parseInt(req.query.limit as string) || 50;

      // Строим дерево MLM структуры для текущего пользователя
      const networkTree = await this.buildRealNetworkTree(userId, 0, new Set(), maxDepth, offset, limit);
  ...
  /**
   * Получить дерево MLM структуры для конкретного пользователя (реальные данные)
   */
   ...
   */
  private async buildRealNetworkTree(userId: number, depth: number = 0, visited: Set<number> = new Set(), maxDepth: number = 16, offset: number = 0, limit: number = 50): Promise<NetworkNode | null> {
    // Предотвращаем бесконечную рекурсию
    if (visited.has(userId) || depth >= maxDepth) {
      return null;
    }
    ...
    // Получаем информацию о наставнике (если есть referrer_id)
    let mentor = null;
    if (user.referrer_id) {
      const mentorResult = await db
        .select()
        .from(users)
        .where(eq(users.id, user.referrer_id))
        .limit(1);
      
      if (mentorResult.length) {
        mentor = {
          userId: mentorResult[0].id,
          firstName: mentorResult[0].first_name,
          username: mentorResult[0].username,
          telegramId: mentorResult[0].telegram_id,
          referralCode: mentorResult[0].referral_code
        };
      }
    }
    ...
    // Получаем общее количество прямых рефералов для пагинации
    const totalReferralsCount = await db
      .select({ count: sql<number>`count(*)` })
      .from(users)
      .where(eq(users.referrer_id, userId));
    
    const totalReferralsNum = totalReferralsCount[0]?.count || 0;

    // Получаем прямых рефералов с пагинацией (только для первого уровня)
    const directReferrals = await db
      .select()
      .from(users)
      .where(eq(users.referrer_id, userId))
      .orderBy(desc(users.created_at))
      .limit(depth === 0 ? limit : 100) // Ограничиваем только на первом уровне
      .offset(depth === 0 ? offset : 0);

    // Рекурсивно строим детей
    const children: NetworkNode[] = [];
    for (const referral of directReferrals) {
      const childNode = await this.buildRealNetworkTree(referral.id, depth + 1, visited, maxDepth, 0, 50);
      if (childNode) {
    ...
    const totalReferralsInNetwork = directReferrals.length + children.reduce((sum, child) => sum + child.network.totalReferrals, 0);
    ...
    return {
      ...
      network: {
        totalReferrals: totalReferralsInNetwork,
