...
import * as productImagesStorage from "./../storage/storage/storage/productsStorage.js";
import { z } from "zod";
  ...
  async uploadImage(req: Request, res: Response) {
    try {
      ...
      if (!validation.success) {
        return res.status(400).json({ error: "Некорректные параметры", details: validation.error });
      }
