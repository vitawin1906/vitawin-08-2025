...
export function TelegramAuthDemo() {
  ...
  const handleDemoAuth = (user: any) => {
    console.log("TelegramAuthDemo: Starting auth for user:", user);
    
    // Устанавливаем пользователя через store (он сам сохранит в localStorage)
    setUser(user);
    console.log("TelegramAuthDemo: User set in store");
    
    // Показываем уведомление об успешной авторизации
    const event = new CustomEvent('auth-success', { 
      detail: { 
        message: 'Вы успешно авторизованы в VitaWin!',
        redirect: 'Перенаправляем вас в личный кабинет...'
      }
    });
    ...
    setTimeout(() => {
      console.log("TelegramAuthDemo: Redirecting to account page");
  ...
  if (!showDemo) {
    return (
      <div className="space-y-4">
        <Button 
          onClick={() => window.open("https://t.me/vitawin_bot", "_blank")}
